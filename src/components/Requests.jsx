    ...
    if (!request || request.length === 0) {
        return (
            <div className="w-full h-screen flex items-center justify-center text-white">
                <h1 className="text-3xl text-center font-mono">No Requests</h1>
            </div>
        );
    }
            ...
            <div className="flex flex-col gap-6 mt-8 w-full max-w-3xl h-auto overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800 p-4">
                {request.map((req) => (
                    ...
                    >
                        <img
                            src={req.photoUrl}
                            alt={`${req.firstName}'s Avatar`}
                            className="w-16 h-16 object-cover sm:w-20 sm:h-20 rounded-full border-2 border-orange-300"
                        />
                        <div className="ml-4 flex flex-col justify-center flex-grow text-center sm:text-left">
                            <h2 className="text-lg text-orange-300">{req.firstName} {req.lastName}</h2>
                            <p className="text-sm text-gray-400">Age: {req.age} | {req.gender}</p>
